  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < slots.length; i++) {
        const s = slots[i];
        await client.query(
          `INSERT INTO routine_slots (routine_id, muscle_group_id, machine_id, slot_order, sets_target, rep_range_min, rep_range_max, tut_target_seconds, modifier)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
          [routine.id, s.muscle_group_id, s.machine_id || null, i + 1, s.sets_target || 1, s.rep_range_min || 8, s.rep_range_max || 12, s.tut_target_seconds || 60, s.modifier || 'NA']
        );
      }
      ...
      if (slots) {
        ...
        for (let i = 0; i < slots.length; i++) {
          ...
          await client.query(
            `INSERT INTO routine_slots (routine_id, muscle_group_id, machine_id, slot_order, sets_target, rep_range_min, rep_range_max, tut_target_seconds, modifier)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
            [req.params.id, s.muscle_group_id, s.machine_id || null, i + 1, s.sets_target || 1, s.rep_range_min || 8, s.rep_range_max || 12, s.tut_target_seconds || 60, s.modifier || 'NA']
          );
        }
      }
      await client.query('COMMIT');
